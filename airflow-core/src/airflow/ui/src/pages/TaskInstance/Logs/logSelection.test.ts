...
describe("getDragClampTarget", () => {
  it("clamps to the last row when the pointer is below and the focus flipped above the anchor", () => {
    ...
    expect(getDragClampTarget({ container, pointerY: 600, selection })).toEqual({
      node: lastRow,
  ...
  it("does not clamp an upward selection while the pointer stays inside the container", () => {
    const container = buildClampContainer();
    const selection = makeDirectionalSelection({
      anchor: getRowTextNode(container, 11),
      anchorOffset: 2,
    ...
    expect(getDragClampTarget({ container, pointerY: 300, selection })).toBeUndefined();
  });

  it("clamps to the first row when the pointer is above and the focus flipped below the anchor", () => {
    const container = buildClampContainer();
    const selection = makeDirectionalSelection({
      anchor: getRowTextNode(container, 11),
      anchorOffset: 2,
      focus: getRowTextNode(container, 12),
      focusOffset: 0,
    });
    const firstRow = container.querySelector('[data-index="10"]') as Element;

    expect(getDragClampTarget({ container, pointerY: 50, selection })).toEqual({
      node: firstRow,
      offset: 0,
    });
  });
  ...
  it("returns undefined when the focus already sits at the clamp target", () => {
    ...
    expect(getDragClampTarget({ container, pointerY: 600, selection })).toBeUndefined();
  });
